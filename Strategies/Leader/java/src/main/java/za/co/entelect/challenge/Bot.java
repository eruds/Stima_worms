...
//        this.previousCommand = gameState.myPlayer.previousCommand.split(" ")[0];
    ...
    public Command run() {
        ...
        // Follows a leader
        int targetId = 2;
        Position leaderPosition = Follow(targetId);
        Position targetWorm = currentWorm.id != targetId ?
                ( leaderPosition == null ? closestWorm.position : leaderPosition) :
                closestWorm.position;

        // The only one that can attack is the leader. If the leader is close enough, the others can start attacking
        if(currentWorm.id == targetId || leaderPosition == null){
