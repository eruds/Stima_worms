...
public class Bot {
    ...
    public Command run() {
        ...
        //getCellToMiddle
        Cell block = getCellToMiddle(currentWorm.position.x, currentWorm.position.y);
        //
    ...
    // fungsi yg ke tengah
    private Cell getCellToMiddle(int c, int d) {
        int a = Math.floorDiv(gameState.mapSize, 2);
        int b = Math.floorDiv(gameState.mapSize, 2);
        int temp;

        List<Cell> surround = getSurroundingCells(c,d);
        Cell ret = surround.get(0);
        int mini = euclideanDistance(a, b, surround.get(0).x, surround.get(0).y);
        for (int i = 0; i < surround.size(); i++) {
            temp = euclideanDistance(a, b, surround.get(i).x, surround.get(i).y);
            if (temp < mini) {
                ret = surround.get(i);
            }
        }
