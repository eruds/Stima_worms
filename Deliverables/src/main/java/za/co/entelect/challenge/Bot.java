...
//        this.previousCommand = gameState.myPlayer.previousCommand.split(" ")[0];
    ...
    public Command run() {
        ...
        int targetId = 1;
        int distance = euclideanDistance(currentWorm.position.x, currentWorm.position.y, closestWorm.position.x, closestWorm.position.y);
        // If there's a worm that is too close, attack that worm.
        Position targetWorm = distance > 5 ? swarmAttack(targetId) : closestWorm.position;
        Position healthpack = lookForHealthPack();
        int distanceToHealthpack =
                healthpack != null ?
                euclideanDistance(currentWorm.position.x, currentWorm.position.y, healthpack.x, healthpack.y)
                        : 100000;
        if(currentWorm.health <= 50 && currentWorm.health >= 20 && healthpack != null && distanceToHealthpack <= 2) {
            targetWorm = healthpack;
        } else {
